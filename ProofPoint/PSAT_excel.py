...
def _normalize_tag_key(cat: str) -> str:
    # lower, drop trailing "_1", replace non-alnum with "_", collapse, trim
    cat = str(cat).lower()
    cat = re.sub(r"_1$", "", cat)
    ...
    if user_tags:
        for tag_category, tag_values in user_tags.items():
            ...
            if isinstance(tag_values, list):
                ...
                user_tags_data[f'{tag_category}_1'] = tag_values
    return user_tags_data


def _mask_email(s: str) -> str:
    """Mask user@domain → u***@domain for debug logs."""
    try:
        local, dom = str(s).split("@", 1)
        if not local:
            return s
...
def fetch_report(report_type: str, api_key: str, seen_id: Optional[str]) -> Tuple[pd.DataFrame, Optional[str]]:
    """
    Fetch a report incrementally; expand usertags for 'users'.
    """
    ...
    stop = False
    rows: List[dict] = []
    debug_ut_samples = 0  # collect a few raw usertags for debugging
    ...
    while next_url and not stop:
        ...
        for it in items:
            ...
            # Flatten scalar attributes; keep usertags JSON (optional)
            ...
            if report_type == "users":
                ...
                # DEBUG: dump a few raw usertags exactly as returned
                if debug_ut_samples < 3:
                    raw_ut = attrs.get("usertags", {})
                    email = row.get("useremailaddress")
                    logger.info("[DEBUG users] raw usertags for %s: %s",
                                _mask_email(email) if email else "<no-email>",
                                json.dumps(raw_ut, ensure_ascii=False)[:1200])
                    debug_ut_samples += 1
    ...
    # DEBUG: show which expanded tag columns actually exist and sample values
    if report_type == "users" and not df.empty:
        cols_to_check = [
            "department_1", "location_1", "office_location_1",
            "manager_email_address_1", "title_1"
        ]
        present = [c for c in cols_to_check if c in df.columns]
        logger.info("[DEBUG users] expanded columns present: %s", present)
        if present:
            try:
                sample = df[["useremailaddress"] + present].head(10).copy()
                sample["useremailaddress"] = sample["useremailaddress"].map(_mask_email)
                logger.info("[DEBUG users] sample values:\n%s", sample.to_string(index=False))
                for c in present:
                    logger.info("[DEBUG users] non-null count for %-26s : %d", c, int(df[c].notna().sum()))
            except Exception as e:
                logger.warning("[DEBUG users] failed to print sample tag values: %s", e)
...
# ======================================
def main():
    logger.info("Starting Proofpoint → Excel dump (test mode).")
    api_key = read_api_key()

    # Fetch reports (no DB connection needed for now)
    results: Dict[str, pd.DataFrame] = {}
    with ThreadPoolExecutor(max_workers=min(4, len(REPORT_TYPES))) as ex:
        future_map = {ex.submit(fetch_report, rt, api_key, None): rt for rt in REPORT_TYPES}
    ...
    # Write each report to Excel
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    for rt, df in results.items():
        if df is None or df.empty:
            logger.warning(f"No data found for report: {rt}")
            continue

        filename = f"{rt}_report_{timestamp}.xlsx"
        try:
            df.to_excel(filename, index=False)
            logger.info(f"✅ Saved {len(df)} rows from {rt} to {filename}")
        except Exception as e:
            logger.error(f"❌ Failed to write {rt} to Excel: {e}")
