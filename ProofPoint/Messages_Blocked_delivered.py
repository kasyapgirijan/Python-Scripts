...
# Function to expand messages and create a row for each threat (works for both blocked and delivered)
...
def extract_proofpoint_data(verbose=False):
    ...
    while current_time < end_time:
        ...
        try:
            ...
            if 'messagesDelivered' in response_data:
                all_messages_delivered.extend(response_data['messagesDelivered'])
            if verbose:
    ...
    # Expand and process both messagesBlocked and messagesDelivered
    expanded_blocked_data = expand_messages(all_messages_blocked)
    expanded_delivered_data = expand_messages(all_messages_delivered)

    # Combine both blocked and delivered data
    combined_data = expanded_blocked_data + expanded_delivered_data
    combined_data = split_threat_time(combined_data)
    ...
    filtered_data = filter_data(combined_data, headers_to_keep)

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    folder_name = f"proofpoint_Email_Security_{timestamp}"
    filename = "Email_security.xlsx"
    save_to_excel(filtered_data, folder_name, filename)
