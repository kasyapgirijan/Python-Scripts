...
def extract_user_tags(attributes):
    ...
    try:
        logger.debug(f"Processing user tags: {user_tags}")
        for tag_category, tag_values in user_tags.items():
            # Clean up category name
            tag_category = tag_category.lower().replace('_1', '')
            
            # Make sure tag_values is a list
            if not isinstance(tag_values, list):
                if tag_values is None:
                    continue
                tag_values = [tag_values]  # Convert single value to list
                
            # Add each tag value with a numbered suffix
            for i, value in enumerate(tag_values):
                if value is not None:  # Skip None values
                    user_tags_data[f'{tag_category}_{i+1}'] = value
    except Exception as e:
        logger.error(f"Error processing user tags: {e}")
        ...
        while True:
            ...
            try:
                ...
                for item in data.get('data', []):
                    ...
                    # Add all attributes as individual columns, except for complex structures
                    for key, value in attributes.items():
                        if isinstance(value, (dict, list)) and key != 'usertags':
                            continue  # Skip nested structures except for user tags
                        item_data[key] = value

                    # For users report type, extract the user tags
                    if report_type == 'users' and 'usertags' in attributes:
                        user_tags = extract_user_tags(attributes)
                        if user_tags:
                            logger.debug(f"Extracted {len(user_tags)} user tags")
                            item_data.update(user_tags)
...
def truncate_tables(db_config, table_names):
    """Truncate specified tables to prevent duplications with correct case handling"""
    try:
        ...
        for table_name in table_names:
            try:
                ...
                # Use double quotes around table name to preserve case
